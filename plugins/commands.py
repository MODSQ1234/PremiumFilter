...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            ...
            InlineKeyboardButton('❌ לסגור הודעה ❌', callback_data='close_data')
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        buttons = [[
            InlineKeyboardButton('➕ סתם כפתור ➕', url=f"https://t.me/{temp.U_NAME}?startgroup=true")
            ],[
            InlineKeyboardButton('💥 עדכונים לבוט 💥', url='https://t.me/+5aoApavIMVo1YzQ1'),
            InlineKeyboardButton('🔎 ערוץ סרטים 🔍', url='https://t.me/TamilanMoviesChat')
            ],[
            InlineKeyboardButton('😅 לקניית בוט כזה לחץ כאן 😊', callback_data='sources')
            ],[
            InlineKeyboardButton('💌 קצת מידע עלינו 💌', callback_data= 'about')
            ],[
            InlineKeyboardButton('❌ לסגור הודעה ❌', callback_data='close_data')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
    ...
    await client.send_cached_media(
        ...
        file_id=file_id,
        caption=f_caption,
        reply_markup=InlineKeyboardMarkup( [ [ InlineKeyboardButton('🚸 הסרט נמחק עוד 10 דק אנא שמורו אותו בהודעות שמורות', callback_data='close_data') ] ] ),
        protect_content=True if pre == 'filep' else False,
        )
